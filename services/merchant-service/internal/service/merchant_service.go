...
func (s *MerchantService) CreateAPIKey(merchantID string, req *model.CreateAPIKeyRequest) (*model.CreateAPIKeyResponse, error) {
	...
	// Hash the key for storage (SHA256 for deterministic lookup)
...
func (s *MerchantService) VerifyAPIKey(apiKey string) (*model.APIKey, error) {
	hash := sha256.Sum256([]byte(apiKey))
	keyHash := hex.EncodeToString(hash[:])

	key, err := s.repo.FindByKeyHash(keyHash)
	if err != nil {
		return nil, err
	}
	if key == nil {
